...
public class RecursionServiceSolution {
	/**
	 *
	 * @param data
	 * @param target
	...
	public static int binarySearch(int[] data, int target, int first, int last) {
		...
		if(target == data[median])
			return median;
		if(target > data[median]) //if it's there, it'd be in the right half of an ascending array
			return binarySearch(data, target, median + 1, last);

		//in the only case remaining, if it's there, it'd be in the left half
		return binarySearch(data, target, first, median - 1);
	}

	//a proxy method for the initial call (first and last assigned appropriate values)
	 ...
	 */
	public static boolean addsUpTo(int[] data, int target, int start) {
		if(target == 0)
			return true;
		if(start == data.length)
			return false;
		return addsUpTo(data, target  - data[start], start+1, indentation + 1) || addsUpTo(data, target, start+1, indentation + 1);
	}
