...
public class SearchService {
	...
	/**
	 * 
	 * @param a (a is in ascending order: a[i] >= a[i-1])
	...
	public static int binarySearch(int[] a, int target) {
		...
		while(first <= last) {
			...
			System.out.println("searching at index "+median+" (iteration "+count+")");
			count++;
			if(target == a[median]) {
				return median;
			}
			if(target > a[median]) {
				first = median + 1;//right half
			}
			else { //target < a[median]
				last = median - 1;//left half
			}
		} //end loop implies first > last
		return -1;
	}
